...
public class RandomItemImpl implements RandomItem {
    ...
    @Override
    public ItemStack getItemStack(Player player, boolean isDebug) {
        return this.getItemStack(player, null, isDebug);
    }
    ...
    public ItemStack getItemStack() {
        ...
        for (int i = 0; i < lores.size(); i++) {
            addStrings(newLores, lores.get(i));
            newLores.set(i, getMessage("&f" + newLores.get(i).replace("\\", "")));
        }
        builder.lore(lores);
    ...
    @Override
    public ItemStack getItemStack(Player player, String pointData, boolean isDebug) {
        RandomItemCallable callable = new RandomItemCallable(player, this.itemData, pointData, isDebug);
