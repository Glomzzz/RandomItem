    ...
    public static NBTCompound translateSection(NBTCompound nbt,
        ...
        for (String key : section.getKeys(false)) {
            ...
            if (obj instanceof ConfigurationSection) {
                base = translateSection(new NBTCompound(), section.getConfigurationSection(key), data, superKey + "  ", isDebug);
            } else if ((base = toNBT(obj, data)) == null) {
                TabooLib.getLogger().warn("Invalid Type: " + obj + " [" + obj.getClass().getSimpleName() + "]");
                continue;
            }
            sendDebug(superKey + "&d  -" + (!notNBT ? ">" : "") + " &b" + key + " &5= &e" + base.toString(), isDebug);
            nbt.put(key, base);
        }
        return nbt;
    }
