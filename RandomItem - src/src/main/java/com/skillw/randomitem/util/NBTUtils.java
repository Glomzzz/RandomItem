...
public final class NBTUtils {
    ...
    public static NBTCompound translateSection(NBTCompound nbt,
        ...
        for (String key : section.getKeys(false)) {
            ...
            if (obj instanceof ConfigurationSection) {
                base = translateSection(new NBTCompound(), section.getConfigurationSection(key), data, superKey + "  ");
            } else if ((base = toNBT(obj, data)) == null) {
                TabooLib.getLogger().warn("Invalid Type: " + obj + " [" + obj.getClass().getSimpleName() + "]");
                continue;
            }
            sendDebug(superKey + "&d  -> &b" + key + " &5= &e" + base.toString());
            nbt.put(key, base);
        }
        return nbt;
    }
    ...
    public static void addCompoundToConfigurationSection(ConfigurationSection nbtSection, NBTCompound compound, String superKey) {
        for (Object object : compound.keySet()) {
            if (object instanceof String) {
                ...
                if (compound.get(object) != null) {
                    addBaseToConfigurationSection(nbtSection, String.valueOf(object), compound.get(object), superKey);
                }
            }
        }
    }

    public static void addBaseToConfigurationSection(ConfigurationSection nbtSection, String key, NBTBase base, String superKey) {
        superKey = superKey != null ? superKey : "";
        if (base == null) {
            return;
        }
        String json = Main.getGson().toJson(base);
        String addition = json.contains("\n") || json.contains("\\n") ? "\n" : "";
        if (base instanceof List) {
            sendDebug(superKey + "&d  -> &b" + key + " &5: &e" + addition + messageToText(json));
            ConfigurationSection listSection = nbtSection.createSection(key);
            int i = 0;
            for (NBTBase nbtBase : base.asList()) {
                addBaseToConfigurationSection(listSection, String.valueOf(i++), nbtBase, superKey + "  ");
            }
        } else {
            sendDebug(superKey + "&d  - &b" + key + " &5: &e" + addition + messageToText(json));
            switch (base.getType()) {
                ...
                case COMPOUND:
                    ...
                    addCompoundToConfigurationSection(compoundSection, nbtCompound, superKey + "  ");
                    break;
                case SHORT:
