        ...
        try {
            if (this.itemStack.hasItemMeta()) {
                ...
                if (itemMeta.hasAttributeModifiers()) {
                    Multimap<Attribute, AttributeModifier> map = itemMeta.getAttributeModifiers();
                    ConfigurationSection section = itemKeySection.getConfigurationSection("attributes");
                    for (Attribute attribute : map.keySet()) {
                        ConfigurationSection attSection = section.createSection(attribute.name());
                        Collection<AttributeModifier> attributeModifiers = map.get(attribute);
                        attributeModifiers.forEach(attributeModifier -> {
                            attSection.set(attributeModifier.getName(), attributeModifier.serialize());
                        });
                    }
                }
