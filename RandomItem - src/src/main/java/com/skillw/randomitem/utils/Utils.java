...
public class Utils {
        ...
        if (replaced.contains(".")) {
            if (replaced.split("\\.").length > 0) {
                ...
                if (section != null) {
                    ...
                    if (section.getDataMap().containsKey(value) || "id".equals(value)) {
                        if ("id".equals(value)) {
                            if (section instanceof Weighable) {
                                final String[] result = new String[1];
                                ((List<BasicData<?>>) section.getDataMap().get("values")).stream()
                                        .filter(basicData -> handleStringsReplaced((List<String>) basicData.getObjects(), data).equals(alreadySectionMap.get(section.getId())))
                                        .forEach(basicData -> result[0] = basicData.getId());
                                return result[0];
                            } else {
    ...
    public static String replaceAll(String string, ComplexData data) {
        ...
        return getMessage(replacePAPI(string, data.getPlayer()));
    }
