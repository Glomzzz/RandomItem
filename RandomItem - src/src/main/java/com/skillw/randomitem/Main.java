...
public final class Main extends Plugin {
    ...
    private static void sendList(CommandSender sender, int page) {
        ...
        for (int i = (page - 1) * number + 1; i <= lastI; i++) {
            ...
            tellrawJson = TellrawJson.create();
            tellrawJson.append(ConfigUtils.getListFormat(i, randomItem));
            tellrawJson.hoverItem(randomItem.getItemStack());
            if (sender instanceof Player) {
                tellrawJson.clickCommand("/ri get " + " " + randomItem.getId());
            }
            tellrawJson.send(sender);
        }
        tellrawJson = TellrawJson.create();
                        ...
                        switch (args[0]) {
                            case "drop":
                                if (args.length >= 10) {
                                    ...
                                    try {
                                        ...
                                        if (args.length == 11) {
                                            isTip = Boolean.parseBoolean(args[10]);
                                        }
                                    } catch (Exception e) {
                                        isSame = false;
                                    }
                                        ...
                                        if (randomItem != null) {
                                            ...
                                            if (world != null) {
                                                ...
                                                if (x != 0 && y != 0 && z != 0) {
                                                    ...
                                                    getItemManager().dropRandomItemRandomly(itemID, location, amountFormula, chanceFormula, isSame, player, sender, true, isTip);
                                                }
                                            } else {
                            ...
                            case "give":
                                if (args.length >= 3) {
                                    ...
                                    String chanceFormula = null;
                                    boolean isSame = false;
                                    boolean isTip = true;
                                    String pointData = null;
                                    if (args.length >= 4) {
                                        amountFormula = args[3];
                                        if (args.length >= 5) {
                                            chanceFormula = args[4];
                                        }
                                        if (args.length >= 6) {
                                            try {
                                                isSame = Boolean.parseBoolean(args[5]);
                                                if (args.length >= 8) {
                                                    isTip = Boolean.parseBoolean(args[7]);
                                                }
                                            } catch (Exception e) {
                                                isSame = false;
                                            }
                                    ...
                                    if (player != null) {
                                        if (amountFormula != null) {
                                            itemManager.giveRandomItemRandomly(itemID, amountFormula, chanceFormula, isSame, player, sender, pointData, true, isTip);
                                        } else {
                                            itemManager.giveRandomItem(player, itemID, sender, pointData, true, isTip);
                                        }
                                        break;
                                    } else {
                            ...
                            case "get":
                                if (args.length >= 2) {
                                    if (sender instanceof Player) {
                                        ...
                                        boolean isTip = true;
                                        if (args.length >= 3) {
                                            amountFormula = args[2];
                                            if (args.length >= 4) {
                                                chanceFormula = args[3];
                                            }
                                            if (args.length >= 5) {
                                                try {
                                                    isSame = Boolean.parseBoolean(args[4]);
                                                    if (args.length >= 7) {
                                                        isTip = Boolean.parseBoolean(args[6]);
                                                    }
                                                } catch (Exception e) {
                                                    isSame = false;
                                                }
                                        ...
                                        if (amountFormula != null) {
                                            itemManager.giveRandomItemRandomly(itemID, amountFormula, chanceFormula, isSame, player, player, pointData, true, isTip);
                                        } else {
                                            itemManager.giveRandomItem(player, itemID, player, pointData, true, isTip);
                                        }
                                    } else {
                                        sendOnlyPlayerMessage(sender);
                                    }
                            ...
                            case "reload":
                                ...
                                break;
                            default:
                                sendValidCommandMessage(sender);
                                break;
                        }
                    }
                }))
                    ...
                    if (args.length > 1) {
                        if ("get".equalsIgnoreCase(args[0])) {
                            switch (args.length) {
                                ...
                                case 7:
                                    return Stream.of("true", "false").filter(s -> s.startsWith(args[6])).collect(Collectors.toList());
                                default:
                                    return Arrays.asList("");
                            }
                        } else if ("give".equalsIgnoreCase(args[0]) || "drop".equalsIgnoreCase(args[0])) {
                            switch (args.length) {
                                case 2:
                                    return null;
                                case 3:
                                ...
                                case 8:
                                    return Arrays.asList("[]");
                                case 9:
                                    return Stream.of("true", "false").filter(s -> s.startsWith(args[8])).collect(Collectors.toList());
                                default:
                                    break;
                            }
                                ...
                                if (args.length == 7) {
                                    ...
                                    if (player != null) {
                                        ...
                                        switch (args.length) {
                                            ...
                                            case 11:
                                                return Stream.of("true", "false").filter(s -> s.startsWith(args[10])).collect(Collectors.toList());
                                            default:
                                                break;
                                        }
                                    } else {
