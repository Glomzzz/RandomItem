    ...
    public BaseSection(String id, Class<? extends BaseSectionType> typeClass, Map<String, Object> map) {
        ...
        if (this instanceof Weighable<?>) {
            ...
            if (!this.getDataMap().containsKey("value-map")) {
                ...
                try {
                    ConcurrentHashMap<String, BasicData<?>> map1 = ((ConcurrentHashMap<String, BasicData<?>>) this.get("value-map"));
                    if (map1.isEmpty()) {
                        right = false;
                    }
                    for (String key : map1.keySet()) {
                        right = map1.get(key) instanceof BasicData<?>;
                        break;
                    }
                } catch (Exception e) {
                    right = false;
            ...
            if (!right) {
                Main.sendMessage(getPrefix() + "&cThe map in the BaseSection must have a key \"value-map\" and its value ConcurrentHashMap<String,BasicData<?>> (can't be empty)! &ein class&6 " + this.getClass().getName());
                this.type = null;
            }
        }
    }
